...
#define CLIENT_BUF_ADDR		0xDEAbd860
				...
				struct wsp_cpmsetbindingsin *bindings_out)
{
	...
	create_setbindings_request(ctx, request, is_64bit, select, cursor);
...
int main(int argc, const char *argv[])
{
	...
	/* set bindings */
	status = create_bindings(talloc_tos(), wsp_ctx, is_64bit, select_stmt, cursor, &bindings_used);
	if (!NT_STATUS_IS_OK(status)) {
