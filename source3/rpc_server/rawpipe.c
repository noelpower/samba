				...
				struct common_rawpipe_loop_ctx *loop_ctx)
{
	...
	struct pipes_struct *p = loop_ctx->conn->p;
	...
	subreq = loop_ctx->reg_ctx->handler(mem_ctx, loop_ctx->conn,
				loop_ctx->init_ctx);
...
static void common_destroy_pipe(struct pipes_struct *p, void *private_data)
{
	struct common_rawpipe_loop_ctx *loop_ctx =
...
static struct tevent_req *start_common_rawpipe_loop(struct dcerpc_ncacn_conn *npc,
						void *private_data);
...
static void rawpipe_process_done(struct tevent_req *subreq)
{
	...
	if (tevent_queue_length(npc->send_queue) > 0) {
		return;
	}
	...
	subreq = common_rawpipe_loop(loop_ctx);
	if (!subreq) {
		goto fail;
	}
	return;
...
void rawpipe_process(struct tevent_req *subreq)
{
	struct common_rawpipe_loop_ctx *loop_ctx =
		tevent_req_callback_data(subreq,
				struct common_rawpipe_loop_ctx);
	struct dcerpc_ncacn_conn *npc = loop_ctx->conn;
	...
	subreq = process_rawpipe_request(npc->p->mem_ctx, loop_ctx);
	if (!subreq) {
		goto fail;
	}
	tevent_req_set_callback(subreq, process_rawpipe_request_done, loop_ctx);
...
static void process_rawpipe_request_done(struct tevent_req *subreq)
{
	...
	struct dcerpc_ncacn_conn *npc = NULL;
	struct _output_data *out;
	NTSTATUS status;
	DEBUG(10,("process_rawpipe_done\n"));
	loop_ctx = tevent_req_callback_data(subreq,
		struct common_rawpipe_loop_ctx);
	npc = loop_ctx->conn;
	out = &npc->p->out_data;
	...
	if (to_send) {
		...
		tevent_req_set_callback(subreq, rawpipe_process_done, loop_ctx);
	} else {
		/*
		...
		struct common_rawpipe_loop_ctx *ctx)
{
	...
	subreq = read_rawpipe_send(ctx->conn, ctx->conn->ev_ctx, ctx->conn->tstream);
	if (!subreq) {
		DEBUG(0, ("Failed to start receving packets\n"));
		goto fail;
	}
	tevent_req_set_callback(subreq, rawpipe_process, ctx);
fail:
	return subreq;
}
						...
						void *private_data)
{
	...
	if (ctx->dtor) {
		npc->termination_fn = common_destroy_pipe;
		npc->termination_data = loop_ctx;
	}
	return common_rawpipe_loop(loop_ctx);
}
				       ...
				       void *init_ctx)
{
	...
	tevent_req_done(req);
	return tevent_req_post(req, npc->ev_ctx);
}
		      ...
		      struct messaging_context *msg_ctx)
{
	...
	if (rpc_rawd_daemon()) {
		...
		ok = dcesrv_setup_ncacn_np_socket("rawpipe", ev_ctx, msg_ctx);
		if (!NT_STATUS_IS_OK(ok)) {
